    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex bg-slate-50 p-1 rounded-lg border border-slate-200 shadow-inner">
            {(["all", "active", "inactive"] as const).map((f) => (
              <button
                ...
                className={`px-4 py-1 text-[8px] uppercase font-black tracking-widest rounded-md transition-all ${
                  statusF === f 
                    ? "bg-blue-600 text-slate-800 shadow-lg shadow-blue-600/20" 
          ...
          <Button className="bg-blue-600 hover:bg-blue-500 text-slate-800 h-9 px-6 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-xl shadow-blue-600/20 active:scale-95 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((agent) => {
          ...
          return (
            <div key={agent.id} className="bg-white backdrop-blur-xl rounded-2xl p-5 border border-slate-200 shadow-xl relative overflow-hidden group hover:bg-slate-50 transition-all">
              <div className="flex items-start justify-between mb-5">
                <div className="flex items-center gap-4">
                  <div className={cn(
                    ...
                    agent.status === 'active' 
                      ? "bg-blue-600/10 text-blue-600 border-blue-600/20 group-hover:bg-blue-600 group-hover:text-slate-800" 
                      : "bg-slate-50 text-slate-700 border-slate-200"
                  )}>
                    {agent.name.split(" ").map(n => n[0]).join("")}
                  </div>
                  <div>
                    <h3 className="text-sm font-black text-slate-800 uppercase tracking-tight leading-none">{agent.name}</h3>
                    <div className="flex items-center gap-2 text-[8px] text-slate-500 font-black uppercase tracking-widest mt-2">
                      <Phone className="h-2.5 w-2.5 text-blue-600/50" /> {agent.phone}
                    </div>
                  </div>
                </div>
                <div className={`h-2 w-2 rounded-full ${agent.status === 'active' ? 'bg-blue-500 animate-pulse shadow-[0_0_8px_hsl(220_26%_48%/0.45)]' : 'bg-slate-100'}`} />
              </div>

              <div className="space-y-4">
                <div className="bg-slate-50 rounded-xl p-3 border border-slate-200/50 shadow-inner">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {agent.areas.map((area) => (
                      <span key={area} className="text-[7px] font-black uppercase tracking-widest bg-white border border-slate-200 px-2 py-0.5 rounded-md text-slate-400">
                        {area}
                      </span>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-blue-600/5 rounded-xl p-3 border border-blue-600/10 shadow-inner">
                    ...
                    <p className="font-mono text-xs font-black text-blue-600 mt-1 tabular-nums">{formatCurrency(agentCollections)}</p>
                  </div>
                  <div className="bg-slate-50 rounded-xl p-3 border border-slate-200/50 shadow-inner">
              ...
              <div className="mt-6 pt-4 border-t border-slate-200/50 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="ghost" size="sm" className="h-7 px-3 rounded-lg text-[8px] font-black uppercase tracking-widest hover:bg-slate-50 text-slate-500 hover:text-slate-800 transition-all">Audit</Button>
                  <Button variant="ghost" size="sm" className={cn(
                    "h-7 px-3 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all",
                    agent.status === 'active' ? "text-red-600 hover:bg-red-50" : "text-blue-600 hover:bg-blue-600/10"
                  )}>
