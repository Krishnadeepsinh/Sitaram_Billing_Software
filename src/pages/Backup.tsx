    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-xl font-semibold tracking-tight text-white">Backup & restore</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="relative space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-950/50 rounded-lg p-3 border border-slate-800/50">
                ...
                <p className="font-mono text-sm font-semibold text-slate-200">Ready</p>
              </div>
              <div className="bg-slate-950/50 rounded-lg p-3 border border-slate-800/50">
                <p className="mb-1 text-[11px] font-medium uppercase tracking-wide text-slate-500">Records</p>
                <p className="font-mono text-sm font-semibold text-slate-200">{(subscribers.length + payments.length + invoices.length + expenses.length + agents.length).toLocaleString()} items</p>
              </div>
            </div>

            <Button 
              onClick={handleExport} 
              className="flex h-11 w-full items-center justify-center gap-2 rounded-lg border-0 bg-blue-600 text-sm font-medium text-white shadow-md shadow-blue-600/25 transition-colors hover:bg-blue-500 active:scale-[0.99]"
            >
        ...
        <div className="bg-slate-900/40 backdrop-blur-xl rounded-xl p-6 border border-slate-800 shadow-2xl relative overflow-hidden group hover:bg-slate-900/60 transition-all">
          ...
          <div className="relative space-y-6">
            ...
            <div className="space-y-1.5">
              <h2 className="text-base font-semibold text-white">Restore from file</h2>
              <p className="text-sm leading-relaxed text-slate-500">
                Replaces all current data with a backup file.
                <span className="ml-1 font-medium text-rose-400">This cannot be undone.</span>
              </p>
            </div>
            ...
            <div className="rounded-lg border-2 border-dashed border-slate-800 bg-slate-950/30 p-6 text-center space-y-3 hover:border-blue-600/50 hover:bg-slate-950/50 transition-all relative group/drop shadow-inner">
              {isImporting ? (
                ...
                <>
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-200">Drop a JSON backup file here</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          { title: "Validated", icon: CheckCircle2, color: "text-emerald-400", desc: "Imports are checked before replacing data." }
        ].map((info) => (
          <div key={info.title} className="bg-slate-900/40 backdrop-blur-xl rounded-xl p-4 border border-slate-800 shadow-xl transition-all hover:bg-slate-900/60">
            ...
            <h3 className="mb-2 text-sm font-semibold text-white">{info.title}</h3>
      ...
      <div className="rounded-xl border border-rose-900/50 bg-rose-950/20 p-6 flex flex-col sm:flex-row gap-4 items-center sm:items-start shadow-xl">
        ...
        <div>
          <h4 className="mb-1 text-sm font-semibold text-rose-400">Warning</h4>
